...
namespace nplm
{
	...
	class propagator {
		...
		public:
		Node<Input_word_embeddings> input_layer_node;  //the input layer
		Node<Linear_layer> first_hidden_linear_node;   //the first hidden linear layer
		Node<Activation_function> first_hidden_activation_node; //the activation for first hidden layer
		Node<Linear_layer> second_hidden_linear_node;  //the second hidden linear layer
		Node<Activation_function> second_hidden_activation_node; //the activation for second layer
		Node<Output_word_embeddings> output_layer_node; //the output layer

		public:
		propagator () : minibatch_size(0), pnn(0) { }

		propagator (model &nn, int minibatch_size) //initialize the model: one_input_layer, two_hidden_layer, one_output_layer
			:
				pnn(&nn),
		...
		template <typename Derived>
			void fProp(const MatrixBase<Derived> &data)
			{
				...
				{
					start_timer(0);
					input_layer_node.param->fProp(data, input_layer_node.fProp_matrix); //from the input data to input embeddings
					stop_timer(0);

					start_timer(1);
					first_hidden_linear_node.param->fProp(input_layer_node.fProp_matrix, 
							first_hidden_linear_node.fProp_matrix); //input_embeddings --> first_hidden_layer_output
				} 
				else
				{
					...
					first_hidden_linear_node.param->fProp(sparse_data,
							first_hidden_linear_node.fProp_matrix); //sparse_input --> first_hidden_layer_output
				}
				first_hidden_activation_node.param->fProp(first_hidden_linear_node.fProp_matrix,
						first_hidden_activation_node.fProp_matrix); //first_hiddeen_layer --> first_hidden_activtion_layer
				stop_timer(1);


				start_timer(2);
				second_hidden_linear_node.param->fProp(first_hidden_activation_node.fProp_matrix,
						second_hidden_linear_node.fProp_matrix); //first_hidden_activation_layer --> second_hidden_layer
				second_hidden_activation_node.param->fProp(second_hidden_linear_node.fProp_matrix,
						second_hidden_activation_node.fProp_matrix); //second_hidden_layer --> second_hidden_activation_layer
				stop_timer(2);
		...
		template <typename DerivedIn, typename DerivedOut>
					...
					double learning_rate, double momentum, double L2_reg) 
			{
				...
				output_layer_node.param->bProp(output,
						output_layer_node.bProp_matrix); //output_embedding_layer_output_error --> output_embedding_layer_input_error 
				stop_timer(7);

				start_timer(8);
				output_layer_node.param->computeGradient(second_hidden_activation_node.fProp_matrix,
						output,
						learning_rate, momentum); //calculate the weight gradient of the edge linking second_hidden_layer_output to output_embedding_layer
				stop_timer(8);
		...
		template <typename DerivedIn>
					...
					double learning_rate, double momentum, double L2_reg) 
			{
				...
				second_hidden_activation_node.param->bProp(output_layer_node.bProp_matrix,
						...
						second_hidden_activation_node.fProp_matrix); //from output backpropagation_matrix --> second_hidden_activation_node_backpropagation_matrix

				second_hidden_linear_node.param->bProp(second_hidden_activation_node.bProp_matrix,
						second_hidden_linear_node.bProp_matrix); //second_hidden_activation_node_backpropagation_matrix --> second_hidden_linear_layer_backpropagation_matrix
				stop_timer(9);
				...
				second_hidden_linear_node.param->computeGradient(second_hidden_activation_node.bProp_matrix,
						...
						learning_rate, momentum, L2_reg); //calculate the weight gradient
				stop_timer(10);
				...
				first_hidden_activation_node.param->bProp(second_hidden_linear_node.bProp_matrix,
						...
						first_hidden_activation_node.fProp_matrix); //from second_hidden_linear_layer_backpropagation_matrix --> first_hidden_activation_layer_backpropagation_matrix

				first_hidden_linear_node.param->bProp(first_hidden_activation_node.bProp_matrix,
						first_hidden_linear_node.bProp_matrix); //first_hidden_activation_layer_backpropagation_matrix --> first_hidden_linear_layer_backpropagation_matrix
				stop_timer(11);
				...
				input_layer_node.param->computeGradient(first_hidden_linear_node.bProp_matrix,
						data,
						learning_rate, momentum, L2_reg); //update the input_embedding_matrix
				stop_timer(13);
