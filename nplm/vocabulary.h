...
namespace nplm
{
	...
	class vocabulary {
		std::vector<std::string> m_words; //list of vocabulary
		boost::unordered_map<std::string, int> m_index; //string to index
		int unk;  //unknown word index
		public:
		vocabulary() 
		{ 
			unk = insert_word("<unk>");
		...
		vocabulary(const std::vector<std::string> &words)
			:
				m_words(words) //initialize the vocabulary according to an existing vocab
		{
		...
		int lookup_word(const std::string &word) const //find index for a word
		{
		...
		int insert_word(const std::string &word) //push a new word
		{
			int i = size();
		...
		template <typename Map>
			int insert_most_frequent(const Map &counts, int vocab_size)
			{
				...
				std::priority_queue<stringint,std::vector<stringint>,compare_second<stringint> > 
					q(compare_second<stringint>(), std::vector<stringint>(counts.begin(), counts.end())); //transform the counts map into a pair vector
		...
		const std::vector<std::string> &words() const { return m_words; } //return vocabulary list
	};
