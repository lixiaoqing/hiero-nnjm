...
// We use:
//       dense2 = dense1^T * sparse (output bProp, input fProp)                     //map the minibatch_size words into embeddings
//       dense1 = sparse * dense2^T (output computeGradient, input computeGradient) //used for backpropagation
// where:
...
class USCMatrix
{
	...
	public:
		Matrix<Index,Dynamic,Dynamic> indexes;  //this matrix (vector) used for storing the indexes of non-zero values in the original sparse matrix
		Matrix<Scalar,Dynamic,Dynamic> values;  //this matrix (vector) used for storing the corresponding values in the original sparse matrix
		int m_rows;
...
void uscgemm(double alpha, const MatrixBase<DerivedA> &a, 
	     ...
	     const MatrixBase<DerivedC> &c_const) //dense matrix a multiplies sparse matrix b, resulting dense matrix c
{
...
void uscgemm(double alpha, 
	     ...
	     const MatrixBase<DerivedC> &c_const) //sparse matrix a multiplies dense matrix b, resulting dense matrix c_const
{
	UNCONST(DerivedC, c_const, c);
	eigen_assert(a.rows() == c.rows());
	eigen_assert(a.cols() == b.rows());
...
void uscgemm_masked(double alpha,
		    ...
		    const MatrixBase<DerivedB> &b,
		    USCMatrix<ScalarC,Index> &c) //dense matrix a multiplies dense matrix b, resulting in a sparse matrix c
{
	eigen_assert(a.rows() == c.rows());
	eigen_assert(a.cols() == b.rows());
	eigen_assert(b.cols() == c.cols());
...
void uscgemv(double alpha, 
	     const USCMatrix<ScalarA,Index> &a,
	     const MatrixBase<DerivedB> &b,
	     const MatrixBase<DerivedC> &c_const) //sparse matrix a multiplies dense vector b, resulting in dense vector c_const
{
	UNCONST(DerivedC, c_const, c);
	eigen_assert(a.rows() == c.rows());
	eigen_assert(a.cols() == b.rows());
