...
void writeNgrams(const vector<vector<string> > &input_data, const vector<vector<string> > &output_data, int ngram_size, const vocabulary &input_vocab, const vocabulary &output_vocab, bool numberize, bool ngramize, const string &filename) //prepare training instances
{
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		ngram_size = arg_ngram_size.getValue();  //ngram order
		...
		add_start_stop = arg_add_start_stop.getValue(); //whether add start and stop symbol
	...
	if (add_start_stop) {
		for (int i=0; i<input_train_data.size(); i++) { //add ngram_size-1 <s> before input_train_data[i], </s> after input_train_data[i]
