...
{ 
	...
	try {
		...
		if (unigram_probs_file.getValue() != "") //unigram_probs_file is ignored, unigram_probs is constructed using training data
		{
	...
	Map< Matrix<int,Dynamic,Dynamic> > training_data(training_data_flat.data(), myParam.ngram_size, training_data_size); //vector to matrix
	...
	Map< Matrix<int,Dynamic,Dynamic> > validation_data(validation_data_flat.data(), myParam.ngram_size, validation_data_size); //vector to matrix
	...
	//ngram_size, input_vocab_size, output_vocab_size, input_embedding_dimenstion, output_embedding_dimension, num_hidden, share_embeddings
	...
	nn.initialize(rng, myParam.init_normal, myParam.init_range, -log(myParam.output_vocab_size)); //model initialization
	nn.set_activation_function(string_to_activation_function(myParam.activation_function)); //set activation function
	loss_function_type loss_function = string_to_loss_function(myParam.loss_function); //set the loss function

	propagator prop(nn, myParam.minibatch_size); //propagation
	propagator prop_validation(nn, myParam.validation_minibatch_size);
	SoftmaxNCELoss<multinomial<data_size_t> > softmax_loss(unigram); //the last layer
	...
	data_size_t num_batches = (training_data_size-1)/myParam.minibatch_size + 1;  //how many batches
	...
	if (myParam.normalization) //normalization for softmax layer
	{
	...
	{ 
		...
		for(data_size_t batch=0;batch<num_batches;batch++)
		{
			...
			double adjusted_learning_rate = current_learning_rate/current_minibatch_size; //update the learning rate
			...
			*/
			...
			prop.fProp(minibatch.topRows(ngram_size-1)); //forward each instance
			...
			if (loss_function == NCELoss)
			{
				...
				minibatch_samples.block(0, 0, 1, current_minibatch_size) = minibatch.bottomRows(1); //target output

				for (int sample_id = 1; sample_id < num_noise_samples+1; sample_id++)    //for each noise sample
					for (int train_id = 0; train_id < current_minibatch_size; train_id++) //for each training instance, collect num_noise_samples noise samples
			...
			else if (loss_function == LogLoss)
			{
				...
				start_timer(4);
				prop.output_layer_node.param->fProp(prop.second_hidden_activation_node.fProp_matrix, scores); //output layer
				stop_timer(4);
				...
				SoftmaxLogLoss().fProp(scores.leftCols(current_minibatch_size), 
						...
						probs, 
						minibatch_log_likelihood); //softmax layer
				stop_timer(5);
				log_likelihood += minibatch_log_likelihood;
				...
				SoftmaxLogLoss().bProp(minibatch.row(ngram_size-1).leftCols(current_minibatch_size), 
						...
						minibatch_weights); //softmax layer
				stop_timer(6);

				prop.bProp(minibatch.topRows(ngram_size-1).leftCols(current_minibatch_size),
						minibatch_weights,
						adjusted_learning_rate, current_momentum, myParam.L2_reg); //output layer
			}
		}
		cerr << "done." << endl;
