...
namespace nplm
{
	...
	void model::resize(int ngram_size,
			...
			int num_hidden,
			int output_embedding_dimension) //reconstruct the neural network
	{
		input_layer.resize(input_vocab_size, input_embedding_dimension, ngram_size-1); //input_vocab_size*(input_embedding_dim*ngram_size)
		first_hidden_linear.resize(num_hidden, input_embedding_dimension*(ngram_size-1));//num_hidden*(input_embedding_dim*ngram_size)
		first_hidden_activation.resize(num_hidden);
		second_hidden_linear.resize(output_embedding_dimension, num_hidden); //output_embedding_dim*num_hidden
		second_hidden_activation.resize(output_embedding_dimension);
		output_layer.resize(output_vocab_size, output_embedding_dimension); //output_vocab_size*output_embedding_dim
	...
	void model::initialize(mt19937 &init_engine, bool init_normal, double init_range, double init_bias) //random initialization
	{
	...
	void model::readConfig(ifstream &config_file)
	{
		...
		while (getline(config_file, line) && line != "")
		{
			...
			else if (fields[0] == "vocab_size") //input and output share the same vocabulary
