...
#include "maybe_omp.h"
#ifdef EIGEN_USE_MKL_ALL  //if EIGEN matrix operation using MKL
#include <mkl.h>
#endif
...
{
    string copy(line); //get a copy of line
...
void readWordsFile(ifstream &TRAININ, vector<string> &word_list)
{
  ...
  while (getline(TRAININ, line) && line != "")
  {
    ...
    if (words.size() != 1) //one word each line
    {
...
void writeWordsFile(const vector<string> &words, ofstream &file)
{
    ...
    {
	file << words[i] << endl; //one word per line
    }
}
...
int setup_threads(int n_threads)
{
    ...
    Eigen::initParallel();
    Eigen::setNbThreads(n_threads); //using eigen for multiple threading
    ...
    #endif
    */
    mkl_set_num_threads(n_threads); //using mkl for multiple threading
    #endif
    #endif
